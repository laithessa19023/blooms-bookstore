...
function AseerBookCard({ book, onAddToCart }) {
  const imageSrc =
  ...
  return (
    <div className="group overflow-hidden rounded-[26px] border border-[#f1e6ea] bg-white shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-2xl">
      ...
      <div className="p-5 text-right space-y-3">
        <h3 className="line-clamp-2 min-h-[52px] text-sm md:text-base font-extrabold leading-7 text-[#2E2A28]">
          {book.title}
        </h3>
        ...
        <div className="pt-2">
          <button
            ...
            className="w-full rounded-full bg-[#4C7A68] py-2.5 text-sm font-bold text-white shadow-sm transition hover:bg-[#3f6757] active:scale-[0.98]"
          >
            🛒 إضافة إلى السلة
          </button>
        </div>
...
function AseerBookCardSkeleton() {
  return (
...
export default function AseerBooksSection() {
  ...
  useEffect(() => {
    const fetchBooks = async () => {
      ...
      if (error) {
        console.error('فشل في جلب كتب دار عصير الكتب:', error.message)
        setBooks([])
      } else {
        setBooks(data || [])
      }
    ...
    <section className="text-right" dir="rtl">
      <div className="mb-6 flex items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">
            أحدث إصدارات دار عصير الكتب المختارة لك
          </p>
        </div>

        <Link
          href="/books?filter=aseer"
      ...
      {loading ? (
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
          {[...Array(4)].map((_, index) => (
            <AseerBookCardSkeleton key={index} />
          ))}
        </div>
      ) : books.length === 0 ? (
